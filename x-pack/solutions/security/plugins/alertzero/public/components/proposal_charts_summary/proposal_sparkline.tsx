...
interface ProposalSparklineProps {
  ...
  color: string;
  ariaLabel: string;
  /** Stable panel identifier used as the @elastic/charts spec id — must not change with locale. */
  panelId: string;
...
export const ProposalSparkline: React.FC<ProposalSparklineProps> = ({
  ...
  // Passed as the series `tickFormat`: @elastic/charts formats tooltip values
  // with the series formatter, falling back to the y-axis one. `Tooltip` itself
  // no longer takes a `valueFormatter`.
