  ...
  async listActivity(
    ...
    const response = await this.deps.storage.search({
      ...
      query: {
        bool: {
          filter: [{ term: { spaceId } }],
          // Pending proposals are always relevant; decided proposals only within
          // the window. A range on `decidedAt` already implies exists, so no
          // separate exists clause is needed.
          // TODO(#19258): once `supersededBy` is added to the index, add
          // `must_not: { exists: { field: 'supersededBy' } }` here so that a
          // replaced proposal no longer appears in either group.
          should: [
            { term: { status: 'pending' } },
